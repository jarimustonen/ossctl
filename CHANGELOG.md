...
- Release cuts now validate required external executables and the exact cargo-dist version
  ...
  bump. Resume re-checks only dependencies needed by the remaining phases, so missing or
  mismatched tooling fails early without mutation (`cargo-dist-preflight`).
...
- Prepare Shipshape Taskfleet convergence release notes (`taskfleet-convergence-release-notes`).
...
- Observe cargo publish workflows through tracked helpers (`observe-cargo-publish-helpers`).
- Verify cargo-dist Homebrew GNU platform stanzas (`verify-homebrew-gnu-platforms`).
