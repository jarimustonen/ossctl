...
<!-- execution-dag:begin -->
```
GLOBAL HEAD-OF-LINE: 🔴🔴 TWO NEW HIGH BUGS — BOTH TO BE FIXED NEXT STINT (Jari, 2026-08-10). Do these first, in this order:
  (1) release-cut-publish-noop (LANE A, HIGH — the critical one): a REAL `ossctl release cut` publish landed NOTHING on crates.io for issuectl 0.8.1 — the engine reported build ok then publish failed on a 300s index-visibility wait for `issuectl-core@0.8.1`, which stayed HTTP 404 forever (never uploaded); `release verify` recorded no receipt; a manual `cargo publish` worked instantly. Signature (cargo "success" + no upload + no receipt) points at the real-cut publish running as a --dry-run/no-op. ⚠️ PUZZLE: ossctl's OWN 0.2.4 cut published fine (verified live), so this may bite the first-publish / downstream / env-specific case — reproduce against a not-yet-published crate. Also file a `--version` drift guard (cut publishes the TREE Cargo.toml version, does NOT bump; `--version` != tree version should error). Makes the engine release path unusable for real downstream cuts → TOP priority.
  (2) publish-crates-yml (LANE C, HIGH): REGRESSION from this stint — the publish-crates-release-trigger fix made publish-crates.yml fire on the version-tag push, but the engine already publishes both crates directly, so CI's `cargo publish -p ossctl-core` hits "already exists" (exit 101) → SPURIOUS RED run on every ossctl cut despite a successful publish (0.2.4 hit it, run 31390837102). Fix: make BOTH cargo publish lines idempotent (treat "already exists" as success), not just the ossctl retry loop.
  --- After those two: deferred/optional — release-verify-delegated-github-release, per-distribution-release, the cargo resume/provenance hardening cluster (cargo-publish-receipt-provenance-resume-safety + homebrew-publish-resume-idempotency). NOTABLE FEATURE (approved, own stint): oss-dist-channel-generator — a NEW /oss-* member via /worktree-make-skill.
LANE A — release engine (crates/ossctl-core/src/release/**; SEQUENCE strictly)
  ...
  ▶ release-cut-publish-noop             (bug, HIGH — NEXT STINT, the critical one. Real `release cut` publish landed NOTHING on crates.io for issuectl 0.8.1: build ok → publish failed on a 300s index-visibility wait for a crate that stayed HTTP 404 forever (never uploaded); no receipt recorded; manual cargo publish worked instantly. Points at the real-cut publish running as a --dry-run/no-op. PUZZLE: ossctl's own 0.2.4 cut published fine (verified live) → repro against a NOT-yet-published crate / downstream repo. Add a --version-vs-tree drift guard too. Cross-ref cargo-per-member-receipts (receipts) + publish-crates-yml (that is a DUPLICATE publish; this is a MISSING publish). Filed from the issuectl 0.8.1 real cut)
...
LANE C — release CI/pipeline infra (.github/workflows/**, dist-workspace.toml — SEQUENCE strictly) — MOSTLY retired for ossctl's own cut, BUT the trigger fix opened one live regression (publish-crates-yml, HIGH) — fix that; the rest stays insurance
  ▶ publish-crates-yml                   (bug, HIGH — NEW, filed 2026-08-10 from the 0.2.4 cut. publish-crates.yml's dep-order step is not idempotent: `cargo publish -p ossctl-core` aborts under set -e on "already exists on crates.io index" (exit 101), because the engine already published core AND the tag-push now also triggers publish-crates.yml. Marks the CI run RED on every ossctl engine cut despite a fully-successful publish. Fix: treat "already exists" as success for BOTH publish lines, not just the ossctl retry loop. Regression from publish-crates-release-trigger — the general idempotency fix also benefits generated projects that don't use the engine)
