...
<!-- execution-dag:begin -->
```
GLOBAL HEAD-OF-LINE: publish-crates-yml (LANE C, HIGH — NEW, the one non-deferred item). 🔴 REGRESSION from this stint: the publish-crates-release-trigger fix made publish-crates.yml fire on the version-tag push, but the ENGINE already publishes both crates directly — so `cargo publish -p ossctl-core` in CI hits "already exists on crates.io index" and exits 101 → every ossctl engine cut now shows a SPURIOUS RED "Publish to crates.io" run even though the publish fully succeeded (0.2.4 hit exactly this, run 31390837102). Fix: make each `cargo publish` in publish-crates.yml idempotent (treat "already exists" as success), applied to the ossctl-core line, not just the ossctl retry loop. Pick this FIRST. --- After it: the rest is deferred/optional — release-verify-delegated-github-release (automate the post-cut GH-Release check), per-distribution-release (finish monorepo support in the engine), the cargo resume/provenance hardening cluster (cargo-publish-receipt-provenance-resume-safety + homebrew-publish-resume-idempotency). NOTABLE FEATURE (approved, own stint): oss-dist-channel-generator — a NEW /oss-* member via /worktree-make-skill.
