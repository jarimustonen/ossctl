...
<!-- execution-dag:begin -->
```
GLOBAL HEAD-OF-LINE: (nothing blocks — pick freely) 🎉 0.2.4 SHIPPED (2026-08-10), all 8 open decisions resolved. Good autonomous pickups: release-verify-delegated-github-release (automate the post-cut GH-Release check), per-distribution-release (finish monorepo support in the engine), or the cargo resume/provenance hardening cluster (cargo-publish-receipt-provenance-resume-safety + homebrew-publish-resume-idempotency) for production-safe interrupted-cut recovery. NOTABLE FEATURE (approved, do in its own stint): oss-dist-channel-generator — a NEW /oss-* member via /worktree-make-skill. Everything is deferred; there is no forced order.
...
LANE C — release CI/pipeline infra (.github/workflows/**, dist-workspace.toml — SEQUENCE strictly) — RETIRED for ossctl's own cut; keep only as insurance
  [DONE stint #15] publish-crates-release-trigger    (fixed — generated/reference publish-crates.yml triggers on version-tag push, not dead release:published; workflow_dispatch fallback kept)
