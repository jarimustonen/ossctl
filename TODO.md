...
<!-- execution-dag:begin -->
```
GLOBAL HEAD-OF-LINE: ✅ Both HIGH bugs FIXED stint #16 (release-cut-publish-noop + publish-crates-yml, both merged & green). NO HIGH blocker remains. ⏸️ ONE DECISION FOR JARI (surfaced stint #16): cut-noop-self-visibility-check — the REAL fix for the critical bug (fail the cut loudly instead of journaling a receipt on a silent no-op upload). It changes behavior on the IRREVERSIBLE publish phase, so the worker deferred it for a maintainer call. Decide: (a) file+build it and fold into 0.2.5, or (b) ship the drift guard alone as 0.2.5 now. The stint #16 landings (--version drift guard) are UNRELEASED on main — 0.2.5 is pending this decision.
  Next after that: deferred/optional — release-verify-delegated-github-release, per-distribution-release, the cargo resume/provenance hardening cluster (cargo-publish-receipt-provenance-resume-safety + homebrew-publish-resume-idempotency). NOTABLE FEATURE (approved, own stint): oss-dist-channel-generator — a NEW /oss-* member via /worktree-make-skill.
LANE A — release engine (crates/ossctl-core/src/release/**; SEQUENCE strictly)
  ...
  ⏸️ cut-noop-self-visibility-check       (bug, HIGH — PROPOSED stint #16, NEEDS JARI'S DECISION. cargo.rs::publish journals a receipt on `cargo publish` exit 0 with NO confirmation the target's own version actually landed on the index → a silent no-op upload fabricates a receipt (exactly the issuectl 0.8.1 signature). Fix = probe the registry for the just-published version before journaling; fail the cut loudly if absent. Changes behavior on the IRREVERSIBLE publish phase (a lagging index would fail the cut) → deliberate maintainer call. NOT YET FILED — file on Jari's go)
