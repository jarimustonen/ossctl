...
<!-- execution-dag:begin -->
```
GLOBAL HEAD-OF-LINE: stint #16 feedback mini-round — Jari APPROVED (2026-08-10) building the real no-op fix + single-source version, then cutting 0.2.5 with all three. In flight (ONE sequenced LANE A worker):
  (1) cut-noop-self-visibility-check (HIGH) — after cargo publish, confirm the target's OWN version landed (bounded index-wait) before journaling the receipt; fail the cut loudly on a silent no-op. Behavior change on irreversible phase — maintainer-approved.
  (2) release-version-single-source — manifest is the single source of truth for the release version; --version becomes an optional must-match confirmation (subsumes the stint #16 drift guard).
  Then CUT 0.2.5 (drift guard + both above). After: deferred/optional — release-verify-delegated-github-release, per-distribution-release, cargo resume/provenance cluster. FEATURE (approved, own stint): oss-dist-channel-generator via /worktree-make-skill.
LANE A — release engine (crates/ossctl-core/src/release/**; SEQUENCE strictly)
  ...
  ▶ cut-noop-self-visibility-check       (bug, HIGH — APPROVED & in flight stint #16. After cargo publish, probe the registry for the target's OWN {name,version} (reuse bounded wait_for_index so normal lag is tolerated) and only journal the receipt once confirmed; fail the cut loudly on a silent no-op. Behavior change on the IRREVERSIBLE publish phase — maintainer-approved 2026-08-10. Built alongside release-version-single-source in one sequenced LANE A worker)
    release-version-single-source        (improvement — APPROVED & in flight stint #16. Manifest = single source of truth for the release version; --version becomes an optional must-match confirmation subsuming the stint #16 drift guard. plan.rs/release.rs. Sequenced WITH cut-noop-self-visibility-check — same LANE A version/publish seam)
