...
description: >-
  OPT-IN architecture-docs member of the /oss-* family. Emits a matklad-style
  ...
  field (its consumer); `/oss-init` is the sole writer of the config value. Reads
  the contract via `ossctl contract show --json --require-approved`. NEVER a
  readiness gate: offered, never required. Thin caller of the `ossctl` binary (the
...
acts on it, but it never writes the config — `/oss-init` is the sole writer of
`OSS-RELEASE.md`.

This skill is a **thin caller** of the `ossctl` binary: the contract is read only through
`ossctl contract show`, never by hand-parsing `OSS-RELEASE.md`. The skill owns only the
...
> `ossctl skill print oss-architecture` to get the skill that ships with the running binary
> before following these steps.

## Two rules that define this member

1. **Opt-in, NEVER a readiness gate.** `/oss-readiness` does **not** fail a repo for lacking
   an `ARCHITECTURE.md`, an ADR log, or a docs site (`ossctl audit` emits no
   architecture/docs gap). These are *offered* — never *required*. This skill runs because a
   human (or `/oss-release`, when the contract opted in) chose to run it; `/oss-release` must
...
- **Making an architectural decision** ("should we adopt X?", "settle A vs B") or **writing
  ...
  a decision.
- **README / value-prop prose** → `/oss-readme`. `ARCHITECTURE.md` is for contributors
  reading the *code*, not users evaluating the *product*.
- **The config itself** (`OSS-RELEASE.md`, including changing the `docs_site` *value*) →
  `/oss-init`. This skill *reads* `docs_site` and *acts on it*; it never edits the contract.
- **Auditing / sequencing** → `/oss-readiness` (scores gaps) / `/oss-release` (orchestrates).

## File ownership — this skill's rows in the family manifest

`/oss-architecture` writes **only** the paths below. It never edits the contract and never
authors a numbered ADR.

| Path | Sole writer | Mutation policy |
|---|---|---|
| `ARCHITECTURE.md` | **`/oss-architecture`** | stage → diff → install; refresh regenerates only the fenced code-map block (below). Never clobber a human-edited file without `--force` (after a scratchpad backup). |
| `docs/decisions/README.md`, `docs/decisions/adr-template.md` | **`/oss-architecture`** | scaffold only; never overwrite without `--force`. The **numbered ADRs** in this dir are written by `/worktree-technical-decision`, not here. |
| docs-site config + its index page (`mkdocs.yml`+`docs/index.md`; `docs/.vitepress/config.*`+`docs/index.md`; `docusaurus.config.js`+`docs/intro.md`; `docs/conf.py`+`docs/index.rst`; `docs.json`+`docs/index.mdx`) | **`/oss-architecture`** | scaffold **only** when `docs_site` ≠ `none`; never clobber an existing config/index without `--force`. |
...
- **Map, don't transcribe.** `ARCHITECTURE.md` is a bird's-eye view: name the major modules,
  ...
  the module map is judgment this skill keeps deliberately in prose, not in `ossctl facts`.)
...
**Resolve the repo root first, then pass it to every `ossctl` call** — this member is
run against the *target*, not necessarily the cwd. Mirror `/oss-init`'s target handling:

```bash
repo_root="$(git -C "$target" rev-parse --show-toplevel)" || exit   # not a git repo → stop
ossctl contract show --json --repo-root "$repo_root" --require-approved || exit
```
...
- **Not a git repo** → **stop**; docs live at a git root, and this skill never `git init`s
  ...
  — architecture docs belong in a project. This is the same guard `/oss-init` applies.
- **Contract gate.** A non-zero exit from `contract show` means no contract, an unreadable
  ...
  "run `/oss-init` first"; `invalid_contract` → surface the validation error and stop;
...
- **Staging convention (identical to `/oss-init`, restated to avoid drift).** All writes go
  to a scratchpad staging dir first: `$SCRATCH/<slug>-staging/`, where `$SCRATCH` is
  `${SCRATCH:-${TMPDIR:-/tmp}}/oss-architecture` (`mkdir -p`) and `<slug>` is the sanitized
...
- **Code map** — wrap this section in HTML fences so a refresh can regenerate *only* it:
  ...
  ```markdown
  <!-- ossctl:code-map:begin — regenerated by /oss-architecture; edit prose outside this block -->
...
- **Opt-in, never a gate.** `/oss-readiness` never fails a repo for missing architecture
...
- **Pass `--repo-root` everywhere.** Resolve the git root of the positional target and give it
  to every `ossctl` call, so approval is checked against the same repo that gets written —
  never the cwd.
...
- **`docs_site: none` ⇒ no site; never edit the value.** Only scaffold when the contract asks;
  changing the `docs_site` value is `/oss-init`'s job, not this skill's.
- **The binary is the source of truth.** The contract is read only through
  `ossctl contract show --json --repo-root <root> --require-approved`; a missing, invalid, or
