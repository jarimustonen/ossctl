...
description: >-
  ...
  fragment directory, add entries to `[Unreleased]`, and — when `/oss-release-cut`
  ...
  fragment source). A thin caller of the `ossctl` binary (the binary is the source
  of truth): it reads the contract via `ossctl contract show`, never re-derives the
  mode from prose. Does NOT bump versions, tag, build, or publish (`/oss-release-cut`);
  does NOT generate README/LICENSE (`/oss-readme`) or CI (`/oss-ci`); does NOT
  write the contract (`/oss-init`). Use for "set up a CHANGELOG", "add a changelog
...
writer** of `CHANGELOG.md` in the `/oss-*` family (design §2.1/§2.2): every operation is a
**structural, marker-anchored** edit, never a regex sweep over free prose. It follows the
[Keep a Changelog](https://keepachangelog.com/) format.

This skill is a **thin caller** of the `ossctl` binary. The deterministic decision — *which
changelog mode this project uses* — lives in the contract and is read via
`ossctl contract show`, never re-derived from prose. The trailer-driven **fragment content**
...
> `ossctl skill print oss-changelog` to get the skill that ships with the running binary
> before following these steps. Read the mode from `ossctl contract show --json`; never
...
- Invoked by **`/oss-release-cut`** as `--finalize --version <v> --date <d>` to move
  `[Unreleased]` under a dated release header and return the compiled notes for the GitHub
  Release body.
- Invoked by **`/oss-release`** (the orchestrator) as the recommended, tier-scaled step
  after CI + README in the bootstrap sequence.

**Do NOT use** for (route elsewhere):
- **Bumping the version, tagging, building, or publishing** → `/oss-release-cut`. This skill
  never edits a manifest version, creates a tag, or touches a registry. It only rewrites
  `CHANGELOG.md` structurally; `/oss-release-cut` *calls* it and consumes its returned notes.
- Generating **README/LICENSE** → `/oss-readme`; **CI** → `/oss-ci`; **CONTRIBUTING/CoC** →
  `/oss-contributing`; **SECURITY.md** → `/oss-security-policy`.
- Writing the **contract** (`OSS-RELEASE.md`) → `/oss-init`. `changelog.mode` is set there;
  this skill *reads* it and never proposes editing it.
- Auditing readiness → `/oss-readiness`.
...
`/oss-changelog` is the **SOLE writer** of `CHANGELOG.md` (design §2.2).

| Path | Sole writer | Mutation policy |
|---|---|---|
| `CHANGELOG.md` | **`/oss-changelog`** | **structural ops only, marker-anchored**; `/oss-release-cut` invokes `--finalize`, it never writes the file itself. |
| `<changelog.fragment_dir>/` (fragment mode only) | **`/oss-changelog`** | create the directory + add fragment files; the compiled output flows into `CHANGELOG.md` at finalize. |
...
- **Read the mode from the binary, gate on exit.** The **first act** is
  `ossctl contract show --json --require-approved || exit` (this skill mutates the repo, so
...
- **Never bump/tag/publish.** This skill's only writes are `CHANGELOG.md` and (fragment mode)
  the fragment directory. Version bumps, tags, and publishes are `/oss-release-cut`'s.
...
| Flag | Default | Effect |
|---|---|---|
| `--finalize` | off | Cut a release: move `[Unreleased]` under a dated version heading and return compiled notes. Requires `--version`. Invoked by `/oss-release-cut`. |
| `--version <v>` | — | The version being finalized (e.g. `1.4.0`). **Required with `--finalize` — if `--finalize` is passed without it, ABORT with a usage error before any write.** The version is `/oss-release-cut`'s §3.4 decision; this skill does not compute the bump. Validate it before use: reject a value containing a newline, `[`/`]`, or `#` (it goes into a `## [<version>]` heading — a malformed value would corrupt the file). |
...
Confirm `ossctl` matches this skill: if `ossctl version --json` reports a `version` different
from **{{CLI_VERSION}}**, re-print the skill (`ossctl skill print oss-changelog`) and follow
...
Invoked by `/oss-release-cut` as the **only** way `CHANGELOG.md` changes at release time
(`/oss-release-cut` never edits the file directly — design §2.1). `--finalize` **requires
...
3. **Return the notes** to the caller (`/oss-release-cut`) as the compiled release-notes
   ...
   ```
   <!-- oss-changelog:notes-start -->
   …release-notes markdown…
   <!-- oss-changelog:notes-end -->
   ```

   The tag itself is created **once** by `/oss-release-cut`'s coordinator — not here.
...
- **Read the mode from `ossctl contract show`; gate on exit.** `--require-approved` because
...
- **`/oss-release-cut` calls `--finalize`; it never writes the file.** This skill returns the
...
- **The binary is the source of truth.** `issuectl changelog` supplies fragment content;
  `ossctl contract show` supplies the mode. This skill hand-parses neither the contract nor
