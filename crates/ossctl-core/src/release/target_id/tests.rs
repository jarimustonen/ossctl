...
fn two_crates_in_one_ecosystem_disambiguate_by_package() {
    ...
    assert_eq!(
        ...
        vec!["rust:ossctl-core", "rust:ossctl"]
    );
}

#[test]
fn same_package_different_channels_disambiguate_by_registry() {
    // `ossctl`'s real four-target contract: one crate published to crates.io, plus
    ...
    assert_eq!(
        journal_target_ids(&targets),
        vec![
            "rust:ossctl-core:crates.io",
            "rust:ossctl:crates.io",
            "rust:ossctl:gh-releases",
            "rust:ossctl:homebrew",
        ]
    );
