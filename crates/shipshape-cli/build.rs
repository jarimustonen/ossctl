//! Build script: capture the current git commit into `SHIPSHAPE_GIT_COMMIT` so
//! `shipshape version` can report it (`AGENTS-AI-FIRST-CLI.md` §10). Falls back to
...
fn main() {
    ...
    println!("cargo:rustc-env=SHIPSHAPE_GIT_COMMIT={commit}");
