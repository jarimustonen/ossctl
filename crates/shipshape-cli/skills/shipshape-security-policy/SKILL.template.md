...
description: >-
  ...
  from `shipshape facts` + `shipshape contract show` and repo inspection, and emits a
  ...
  workflow); the CI quality-gate workflows are /shipshape-ci's. Templated emission + threat-signal
  detection judgment; a thin caller of the `shipshape` binary (the binary is the
...
This skill is a **thin caller** of the `shipshape` binary. The deterministic evidence — ecosystems,
maturity, and the `targets` map (which carries the shipped-binary signal) — comes from
`shipshape facts` and `shipshape contract show`. This skill owns only the **judgment**: reading the
...
> `shipshape skill print shipshape-security-policy` to get the skill that ships with the running binary
> before following these steps. Read the project's dials via `shipshape contract show --json` —
...
- As the security step of a `/shipshape-release` bootstrap run (after `/shipshape-contributing`).
...
- **CI-security tooling** — CodeQL, `zizmor`, `actionlint`, Dependabot/Renovate, the OpenSSF
  **Scorecard workflow as a CI gate** → `/shipshape-ci`. This member writes the security *document*,
...
- **Publish-time signing / provenance / SLSA / cosign attestations** → `/shipshape-release-cut`. The
  `provenance_level` dial is shared; this member only *documents* it, never performs signing.
- **CONTRIBUTING / Code of Conduct / issue templates** → `/shipshape-contributing`. Security reporting
  is a distinct document from contribution onboarding (the sibling boundary).
- **Generating the config** (`OSS-RELEASE.md`) → `/shipshape-init`. This member reads the config; it
  never writes it.
...
`/shipshape-security-policy` is the **sole writer** of `SECURITY.md`.

| Path | Sole writer | Mutation policy |
|---|---|---|
| `SECURITY.md` | **`/shipshape-security-policy`** | full-file on first gen; on refresh the composition reads the existing file as data and rewrites only the **marker-anchored regions** (`<!-- shipshape-security:supported-versions-start/-end -->`), preserving text outside them; never overwrite a **markerless** (hand-authored) `SECURITY.md` without `--force`, after a scratchpad backup. |
| `.github/workflows/scorecard.yml` | **`/shipshape-security-policy`** | written **whenever** `scorecard` ∈ `health_badges` (its badge producer — a floor, not production-gated). Obeys the **same** stage → install / never-clobber / symlink-refuse / scratchpad-backup rules as `SECURITY.md`. Distinct from `/shipshape-ci`'s `codeql.yml`/`zizmor.yml` — do not touch those. |

No other member writes `SECURITY.md`. This skill writes no CI **quality-gate** workflow
(`ci.yml`/`codeql.yml`/`zizmor.yml` are `/shipshape-ci`'s), no `LICENSE`, no `CONTRIBUTING.md`, and
...
|---|---|---|
| 1 | **Network / socket / HTTP input** | source references to sockets, listeners, HTTP servers/clients, or a bound port (`TcpListener`, `reqwest`/`hyper`/`axum`, `net/http`, `http.server`, `fetch`, `express`). A tool that *only* calls `shipshape`/`git` locally is not a network service. |
...
|---|---|---|
| `spike` | no | **skip** — write nothing; advise only (`/shipshape-security-policy` is an mvp+ member; over-scaffolding a spike is the anti-pattern). |
...
here means `${SCRATCH:-${TMPDIR:-/tmp}}/shipshape-security-policy` (`mkdir -p` it; slug + counter on
collision).
...
A non-zero exit stops the run: no config (`/shipshape-init` must run first), a `status: draft` config
...
Confirm `shipshape` is on `PATH` (if not, stop and tell the human to install it — never hand-derive
the contract) and that it matches this skill; on a `version` mismatch, re-print
(`shipshape skill print shipshape-security-policy`) and follow that copy.
...
- **Marker-anchored refresh.** When refreshing a `SECURITY.md` this skill previously wrote (its
  ...
  `<!-- shipshape-security:… -->` regions — installing it rewrites only the owned regions, so `--force`
...
- **The next step is the human's:** review the policy, confirm the disclosure contact and SLA
  windows, then commit it. `/shipshape-security-policy` STOPS here.
...
- **Stay in your lane.** SECURITY.md and disclosure prose only — CI-security lints are `/shipshape-ci`;
  publish-time signing/provenance is `/shipshape-release-cut`; contribution docs are `/shipshape-contributing`.
- **The binary is the source of truth.** Facts come from `shipshape facts`; the config is read via
  `shipshape contract show --require-approved`, never hand-parsed. Gate on the exit code.
