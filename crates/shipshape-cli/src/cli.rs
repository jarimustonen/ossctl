...
pub(crate) const GIT_COMMIT: &str = env!("SHIPSHAPE_GIT_COMMIT");
...
#[command(
    ...
    about = "Release & readiness coordinator for the /shipshape-* skill family.",
    long_about = "shipshape takes any repo to OSS release quality and cuts releases from it. \
It owns the OSS-RELEASE.md contract normalizer, repo-fact detection, the readiness \
audit, and the resumable per-ecosystem release-cut engine. The prose /shipshape-* skills \
...
enum Command {
    ...
    /// List, install, or print the companion /shipshape-* AI-skills.
    Skill {
        #[command(subcommand)]
        action: SkillAction,
    },
...
/// `shipshape version` — the §10/§17 version surface. Emits `version`, `commit`,
/// `schema_version`, `supported_schemas`, and the bundled-skill catalog.
fn cmd_version(format: OutputFormat) -> Result<(), CliError> {
    let payload = VersionPayload {
        version: CARGO_VERSION,
        commit: GIT_COMMIT,
        schema_version: shipshape_core::SCHEMA_VERSION,
        supported_schemas: shipshape_core::SUPPORTED_SCHEMAS,
    ...
    match format {
        ...
        OutputFormat::Text => {
            crate::output::stdoutln!("shipshape {}", payload.version)?;
