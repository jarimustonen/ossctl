//! `shipshape dist generate` handler.
//!
...
//!    [`shipshape_core::dist`] generator (cross-platform by default — macOS AND
...
use shipshape_core::contract::schema::{DistributionAdapter, Status};
use shipshape_core::contract::{self, LoadError, Normalized};
use shipshape_core::ports::CommandRunner;
...
/// Arguments for `shipshape dist generate`.
#[derive(Args, Debug)]
...
/// `shipshape dist generate` — generate the cargo-dist release infra from the
...
pub fn generate(
    ...
    let generated = shipshape_core::dist::generate(distribution);
    ...
    let workflow = if args.no_workflow {
        warnings.push(format!(
            "skipped the workflow step (--no-workflow); {DIST_CONFIG_FILENAME} was written but \
             {RELEASE_WORKFLOW_PATH} was NOT regenerated — re-run `shipshape dist generate` (without \
            ...
            CliError::system(
                ...
                format!(
                    "wrote {DIST_CONFIG_FILENAME}, but the `dist` (cargo-dist) tool is not \
                     ...
                     `shipshape dist generate`, or pass --no-workflow to skip this step",
                    pinned = shipshape_core::dist::PINNED_CARGO_DIST_VERSION,
                ),
            )
        } else {
            CliError::system(
    ...
    if !workflow_path.is_file() {
        return Err(CliError::system(
            ...
            format!(
                ...
                shipshape_core::dist::PINNED_CARGO_DIST_VERSION,
            ),
        ));
    }
    Ok(())
