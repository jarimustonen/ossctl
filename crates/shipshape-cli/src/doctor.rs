//! `shipshape doctor` — read-only self-diagnostic (`AGENTS-AI-FIRST-CLI.md` §18).
//!
...
/// Arguments for `shipshape doctor`.
#[derive(Args, Debug)]
...
fn run_checks() -> Vec<CheckResult> {
    vec![CheckResult {
        id: "binary.self",
        status: Status::Ok,
        message: format!("shipshape {} responding", env!("CARGO_PKG_VERSION")),
        fix_suggestion: None,
    }]
}
