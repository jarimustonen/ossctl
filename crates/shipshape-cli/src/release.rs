//! `shipshape release …` handlers.
//!
//! The full verb set (`plan`/`cut`/`resume`/`verify`/`show`/`list`/`abandon`) is
//! implemented here as a thin dispatcher over `shipshape-core::release`: the plan
...
pub struct PlanArgs {
    ...
    /// Permit planning shipshape itself with a binary built from a different commit.
    /// This is an explicit escape hatch for a deliberate shipshape self-cut.
    #[arg(long)]
...
pub struct CutArgs {
    ...
    #[arg(long, value_name = "LEVEL")]
    pub bump: Option<String>,
    /// Permit cutting shipshape itself with a binary built from a different commit.
    /// This is an explicit escape hatch for a deliberate shipshape self-cut.
    #[arg(long)]
    pub allow_stale_binary: bool,
}
...
///
/// A release plan derives its version and targets from the live tree, so a Shipshape
...
/// when that tree's `origin` identifies the canonical shipshape repository.
...
/// `shipshape release plan` — compute and seal a content-addressed release plan
...
/// seals the plan in `shipshape-core::release::plan` — so `plan`, the audit, and
/// `/shipshape-init` agree on the contract down to the byte (ADR-0001 §3). Emits the
...
/// `shipshape release verify` — read-only reconcile of a journaled run against
...
/// `shipshape release show <run_id>` — the §12 progress query for a release run:
...
/// One run's summary row in `release list` — enough for the `/shipshape-release`
...
struct RunListBody {
    ...
    /// shipshape). Surfaced explicitly rather than dropped: one of these could be an
...
/// `shipshape release list` — enumerate every release run under the journal root with
/// its status (ADR-0003 §3), for the `/shipshape-release` skill's in-flight-run gate.
///
...
/// `shipshape release abandon <run_id>` — terminally mark a non-terminal run
...
/// `shipshape release resume <run_id>` — reconcile an interrupted run against remote
...
/// the sealed commit), or a new release planned. `shipshape` will not continue a
...
/// resuming; `shipshape` never overwrites or blind-re-publishes.
...
/// `shipshape release cut --plan <id>` — execute a sealed plan across the
...
pub fn cut(args: &CutArgs, format: OutputFormat) -> Result<(), CliError> {
    ...
    let target_ids = shipshape_core::release::journal_target_ids(&current.targets);
...
///
/// Thin wrapper over [`shipshape_core::release::plan::resolve_release_version`] used by
...
/// Map a [`VersionResolveError`](shipshape_core::release::plan::VersionResolveError) to
/// the §10 error envelope with an actionable, operator-facing message.
///
/// `shipshape release cut` publishes the version already in the manifest — it does not
...
/// [`BumpLevel`](shipshape_core::protocol::plan::BumpLevel), or `None` when the
...
/// Map a strict-semver [`BumpError`](shipshape_core::release::bump::BumpError) from the
...
/// [`BumpPlan`](shipshape_core::protocol::plan::BumpPlan) directly (the caller guards
...
/// single-distribution common case (`len <= 1`, incl. shipshape itself) is
/// unaffected.
...
mod tests {
    ...
    fn canonical_origin_identifies_only_the_configured_source_repository() {
        // ADR-0005 renames the product but deliberately does not rename this GitHub
        // repository. Self-cut detection follows package metadata, never product-name
        // guessing.
