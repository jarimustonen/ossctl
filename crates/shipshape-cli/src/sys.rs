//! Concrete, production implementations of the `shipshape-core` effect ports.
//!
//! `shipshape-core` domains take the [`shipshape_core::ports`] traits by reference so
...
/// waits remain real; `SHIPSHAPE_E2E_FAST_CLOCK=1` lets a compiled-binary test drive
...
impl Clock for RealClock {
    ...
    fn sleep(&self, dur: Duration) {
        // The acceleration hook is compiled into test/debug binaries only. A release
        // Shipshape binary must never let an ambient test variable collapse safety
        // polling windows.
...
/// This is deliberate: `shipshape` takes no `rand`/`ulid` dependency (the workspace
...
/// private registry/mirror the old `npm view` would have consulted. For shipshape's
...
///
/// [`VerifyOutcome::Unknown`]: shipshape_core::protocol::release::VerifyOutcome::Unknown
...
/// `flock`; this impl instead uses an `O_EXCL` lock *file* so `shipshape` takes no
...
mod tests {
    ...
    fn sparse_index_path_follows_cargo_prefix_convention() {
        ...
        // shipshape's own crates — the dogfood targets.
        assert_eq!(
            RealRegistryQuery::sparse_index_path("shipshape-core"),
            "sh/ip/shipshape-core"
        );
        // The name is lowercased; hyphens are preserved. The frozen legacy crate
        // remains a valid registry coordinate (ADR-0005).
        assert_eq!(
