//! Integration tests for the `shipshape` binary surface (`version`, `doctor`,
...
fn version_flag_aliases_version_verb() {
    let text = shipshape().arg("version").output().unwrap();
    assert!(text.status.success(), "version must exit 0");
    for flag in ["--version", "-V"] {
        let alias = shipshape().arg(flag).output().unwrap();
    ...
    let json = shipshape().args(["version", "--json"]).output().unwrap();
    assert!(json.status.success(), "version --json must exit 0");
    for flag in ["--version", "-V"] {
        for args in [[flag, "--json"], ["--json", flag]] {
            let alias = shipshape().args(args).output().unwrap();
...
fn version_alias_does_not_mask_subcommands_or_release_flags() {
    for flag in ["--version", "-V"] {
        let out = shipshape().args([flag, "doctor"]).output().unwrap();
    ...
    for args in [
        ...
        let out = shipshape().args(args).output().unwrap();
...
fn assert_json_help_tree(path: &[String]) {
    ...
    let mut expected_path = vec!["shipshape".to_string()];
    ...
    assert_eq!(
        command["name"],
        path.last().map_or("shipshape", String::as_str),
...
fn doctor_json_shape() {
    let out = shipshape().args(["doctor", "--json"]).output().unwrap();
...
fn unknown_subcommand_is_structured_error() {
    let out = shipshape().arg("frobnicate").output().unwrap();
...
/// contract-location provenance explicit rather than pretending shipshape has a
...
fn exit_codes_distinguish_not_found_operational_fault_and_success() {
    ...
    shipshape().arg("version").assert().success();
}

/// The `/shipshape-init` skill's whole "stage → validate → install" lifecycle rests on
    ...
    for name in [
        ...
        assert_eq!(
            ...
            "{name}: validate verdict must be identical at a bare staging root and a populated \
             real repo root (the staging-root invariant the shipshape-init skill depends on)"
        );
    }
}
...
/// envelope with the SCHEMA.md §4 field names shipshape-init relies on. A fresh temp
...
fn facts_json_shape_and_field_names() {
    ...
    let d = &v["data"];
    // The exact field names shipshape-init reads (SCHEMA.md §4).
    for key in [
        "ecosystems",
        "packages",
        "cargo_publish",
...
fn release_verify_reconciles_a_journaled_run() {
    ...
    let out = shipshape()
        ...
        .env("SHIPSHAPE_TEST_GH_CALLS", &calls)
        .output()
        .unwrap();
...
/// gate field the `/shipshape-release` skill keys on.
#[test]
...
/// A synthetic canonical shipshape tree's HEAD intentionally differs from the test
...
#[test]
fn release_plan_allows_downstream_and_forked_shipshape_trees_without_escape_hatch() {
    let dir = approved_git_repo();
    set_origin(&dir, "https://github.com/someone/shipshape.git");
