...
impl TempRepo {
    pub fn new(status: &str) -> Self {
        ...
        .expect("write source");
        repo.git(&["init"]);
        repo.git(&["config", "user.name", "shipshape e2e"]);
    ...
    pub fn run(&self, shims: &Shims, args: &[&str]) -> Output {
        let mut command = Command::new(env!("CARGO_BIN_EXE_shipshape"));
        command
            .args(args)
            ...
            .env("SHIPSHAPE_E2E_FAST_CLOCK", "1");
        command.output().expect("run shipshape")
    }
