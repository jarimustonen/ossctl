    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NotFound(p) => write!(
                f,
                "no {CONTRACT_FILENAME} at {} (run /shipshape-init to generate one)",
                p.display()
            ),
...
fn build(map: &Mapping, p: &mut Problems, repo_root: &Path, fs: &dyn Fs) -> Contract {
    ...
    // maturity — required (inference is /shipshape-init's job, not the normalizer's).
    let maturity = match map.get("maturity") {
        None => {
            p.err(
                "maturity is required (spike|mvp|production) — /shipshape-init infers it"
                    .to_string(),
            );
            Maturity::Mvp
        }
        ...
        && !fs.is_dir(&repo_root.join(&changelog.fragment_dir))
    {
        p.warn(format!(
            "changelog.mode 'fragment' but the fragment dir {} does not exist yet under {} — \
             /shipshape-changelog creates it; /shipshape-readiness reports it as a gap until then",
            quote_for_diagnostic(&changelog.fragment_dir),
            repo_root.display()
        ));
    }
        ...
        })
    {
        p.err(
            "floor: dist-workspace.toml publish-jobs includes 'homebrew', but the contract's \
             Homebrew target uses adapter 'homebrew-tap' — cargo-dist CI and shipshape would both \
...
fn parse_one_distribution(m: &Mapping, schema_version: u32, p: &mut Problems) -> Distribution {
    ...
    // /shipshape-init's job, exactly as for `maturity`. A bare `distribution: {}` is
        ...
        None => {
            p.err(
                "distribution.adapter is required when a distribution block is present \
                 (cargo-dist|goreleaser|manual) — /shipshape-init infers it"
                    .to_string(),
            );
        ...
        match b {
            ...
            HealthBadge::Coverage if maturity != Maturity::Production => p.err(format!(
                "floor: health_badge 'coverage' has no producer — the coverage gate is a \
                 production-tier /shipshape-ci output; current maturity is '{}'",
                maturity.as_str()
            )),
    ...
    for target in targets.iter().filter(|t| {
        ...
        p.err(format!(
            "floor: targets declares a crates.io publish for {} with adapter {}, but {} \
             ...
             Run 'shipshape facts --json' from the repository root and inspect \
             data.cargo_publish to see the manifest evidence shipshape read.",
...
/// tap serve a named-package `homebrew-tap` target (shipshape's OWN contract shape) —
...
mod tests {
    ...
    fn expanded_crates_io_target_contradicted_by_publish_false_is_an_error() {
        ...
        assert!(
            n.problems
                .errors
                .iter()
                .any(|e| e.contains("shipshape facts --json") && e.contains("data.cargo_publish")),
    ...
    /// Round-trip: the serialized JSON shape a downstream `/shipshape-*` member reads.
    #[test]
    ...
    fn dist_workspace_homebrew_publish_job_refuses_an_engine_owned_tap_target() {
        ...
        assert_error_contains(
            &n,
            "cargo-dist CI and shipshape would both write the same tap",
        );
    }
    ...
    /// engine-owned target remains valid (shipshape's own release shape).
    #[test]
    ...
    fn ci_delegated_homebrew_rejects_a_dist_workspace_tap_mismatch() {
        ...
        assert_error_contains(
            &n,
            "cargo-dist would write one tap while shipshape verifies another",
        );
    }
    ...
    fn homebrew_truth_table_all_eight_rows() {
        ...
        let rows = [
            ...
            (true, false, true, true),   // 6: tap + tap-target → well-formed (shipshape's case)
    ...
    fn ci_delegated_homebrew_target_requires_tap_for_ci_and_verify() {
        let text = "---\nstatus: approved\nmaturity: production\necosystems: [rust]\ntargets:\n  \
                    - {ecosystem: rust, package: shipshape, registry: homebrew, adapter: cargo-dist}\n---\n";
    ...
    /// Regression for `intake-feature-shipshape-73e870268475`: the field-reported
    ...
    /// producer is the well-formed case (shipshape's own shape) — clean.
    #[test]
    ...
    fn homebrew_registry_requires_homebrew_adapter() {
        let text = "---\nstatus: approved\nmaturity: production\necosystems: [rust]\ntargets:\n  \
                    - {ecosystem: rust, package: shipshape, registry: homebrew, adapter: manual}\n\
                    distribution:\n  adapter: cargo-dist\n  homebrew_tap: owner/tap\n---\n";
        assert_error_contains(
            &norm(text),
    ...
    fn homebrew_core_target_needs_no_tap() {
        let text = "---\nstatus: approved\nmaturity: production\necosystems: [rust]\ntargets:\n  \
                    - {ecosystem: rust, package: shipshape, registry: crates.io, adapter: cargo-publish}\n  \
                    - {ecosystem: rust, package: shipshape, registry: homebrew, adapter: homebrew-core}\n---\n";
        let n = norm(text);
        assert!(n.is_valid(), "errors: {:?}", n.problems.errors);
    }
    ...
    fn homebrew_registry_omitted_adapter_is_a_floor() {
        let text = "---\nstatus: approved\nmaturity: production\necosystems: [rust]\ntargets:\n  \
                    - {ecosystem: rust, package: shipshape, registry: homebrew}\n---\n";
        assert_error_contains(
            &norm(text),
            "requires adapter 'homebrew-tap' (personal tap), 'homebrew-core'",
        );
    ...
    fn homebrew_tap_target_satisfied_via_plural_distributions() {
        let text = "---\nstatus: approved\nmaturity: production\necosystems: [rust]\ntargets:\n  \
                    ...
                    - {ecosystem: rust, package: shipshape, registry: homebrew, adapter: homebrew-tap}\n\
                    distributions:\n  \
                    - {package: shipshape, adapter: cargo-dist, installers: [shell], homebrew_tap: owner/tap}\n---\n";
        let n = norm(text);
        assert!(n.is_valid(), "errors: {:?}", n.problems.errors);
        assert!(
            !n.problems
    ...
    fn shipshape_own_contract_shape_no_cross_check_warning() {
        ...
        assert!(
            !n.problems
                ...
                .iter()
                .any(|w| w.contains("nothing to install")),
            "shipshape's own shape must not cross-check warn: {:?}",
            n.problems.warnings
        );
    }
    ...
    /// shipshape's own (correct) contract shape.
    #[test]
