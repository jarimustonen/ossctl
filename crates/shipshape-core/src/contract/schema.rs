...
//! JSON contract every `/shipshape-*` member reads (SCHEMA.md §4, preserved
...
wire_enum! {
    /// Machine-readable approval gate (SCHEMA.md §1). `/shipshape-init` writes `draft`
...
wire_enum! {
    ...
    /// Required — inference is `/shipshape-init`'s job, not the normalizer's.
...
wire_enum! {
    /// Contributor sign-off requirement, read by `/shipshape-contributing`.
...
wire_enum! {
    /// Which dependency-update bot `/shipshape-ci` emits.
...
pub struct Contract {
    ...
    /// Whether `/shipshape-release-cut` may derive the bump from commit types.
