//! Core library for `shipshape` — the deterministic engine behind the `/shipshape-*`
//! Claude Code skill family.
//!
//! All logic that must be exact and identical for every caller lives here so it
//! is unit-testable without spawning the binary. The `shipshape-cli` crate is a
...
//!
//! `shipshape-core` is the canonical library surface, so public items must carry
