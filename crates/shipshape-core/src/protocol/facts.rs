...
//!
//! This is the versioned surface `/shipshape-init` (config generation) and the
...
//! the rest — because the prose `/shipshape-init` skill already relies on those exact
...
/// `shipshape facts` and consumed directly by internal release and audit logic.
///
...
pub struct Facts {
    ...
    /// the shared facts schema `/shipshape-init` and `audit` read; the plan content-addresses
...
// `package` is the field name `/shipshape-init` reads (SCHEMA.md §4); the
...
pub struct MaturitySignals {
    ...
    /// are presence/name heuristics, not proofs — `/shipshape-init` presents them to a
