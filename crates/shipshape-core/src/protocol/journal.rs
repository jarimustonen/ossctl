...
//!
//! The journal is durable across `shipshape` upgrades — a run started under one
...
/// versions the *durable* on-disk journal, which must survive `shipshape` upgrades.
...
/// rule — a v1 `shipshape` refuses a v2 line rather than misreading it). A v2 run
