...
//! `shipshape-core` can refactor internals without a wire break (ADR-0001 §2).
//!
...
/// Current envelope/DTO schema version for `shipshape`'s public JSON output.
///
...
///
/// Surfaced by `shipshape version --json` so a caller can detect drift between its
