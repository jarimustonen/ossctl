...
//! (`dep = "=X.Y.Z"`, the shape `/shipshape-init` emits) **cannot be `cargo package`d
...
//! declared target** (which is what `/shipshape-init` emits). A multi-crate workspace
...
impl CargoAdapter {
    ...
    ///
    /// crates.io is the only rust registry shipshape supports today, so any other
