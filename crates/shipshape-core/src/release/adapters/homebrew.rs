...
//! `homebrew-tap` formulas must also carry shipshape's ownership marker; CI-delegated
...
/// The stable, greppable prefix of the **ownership marker** every shipshape-generated
/// formula carries as its first line. Its presence is how the *tap-write* path tells
/// a Shipshape-managed formula (safe to fully regenerate) from a hand-maintained one
...
/// reads as "shipshape-managed". See [`render_formula`] (writer) and
/// [`formula_carries_marker`] (reader).
const FORMULA_MARKER_PREFIX: &str =
    "# Generated by shipshape; do not edit by hand (template-version:";
/// Legacy marker emitted by ossctl. It remains trusted so a renamed installation can
/// safely resume and update formulas created before the product migration (ADR-0005).
...
/// still identifies the file as shipshape-managed regardless of the integer.
const FORMULA_TEMPLATE_VERSION: u32 = 2;

/// Whether `bytes` carry the shipshape ownership marker on their **first line** —
...
/// `__END__`/patch data must NOT be mistaken for shipshape output and fully regenerated
...
/// marked formula is shipshape-managed → safe to fully regenerate; an unmarked one is
...
impl HomebrewAdapter {
    ...
    /// carries the shipshape ownership marker (see [`render_formula`]), it *is* shipshape
    ...
    /// this path refuses with instructions to add the ownership marker deliberately.
    /// It never silently claims or rewrites hand-authored content. The reader accepts
    /// both Shipshape and legacy ossctl ownership markers (ADR-0005).
    ///
    /// **Idempotent.** It compares the regenerated content against the tap's current
    /// bytes; an exact match is a clean no-op
    ...
    fn run_tap_write(
        ...
        //    formula Shipshape or ossctl generated (carrying either recognized marker)
        //    is safe to fully regenerate. An unmarked, hand-maintained formula must NOT
        //    be clobbered. The actionable refusal below requires the maintainer to add
        //    the marker explicitly before Shipshape claims full ownership.
...
/// cargo-dist's CI-owned formulas have no shipshape marker by design.
...
/// `condition`. shipshape's renderer uses one combined condition, while cargo-dist
...
/// concatenated (`my-tool` → `MyTool`, `shipshape` → `Shipshape`). A small, faithful
