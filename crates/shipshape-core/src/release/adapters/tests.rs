...
fn cargo_rejects_a_non_crates_io_registry_before_any_action() {
    // crates.io is the only rust registry shipshape supports. A cargo-publish target
...
/// `shipshape-homebrew-<name>-<version>-<pid>-<nanos>`) by scanning `temp_dir` for
...
fn read_created_formula(name: &str, version: &str) -> (std::path::PathBuf, String) {
    let prefix = format!("shipshape-homebrew-{name}-{version}-");
...
fn homebrew_tap_direct_writes_when_the_formula_already_exists() {
    ...
    let cmd = FakeCmd::new()
        // The fake clone checks out an older *shipshape-marked* formula so the rendered
    ...
    assert!(formula.contains("bin.install \"hbwrite\""), "{formula}");
    assert!(
        formula.contains(
...
fn homebrew_render_formula_carries_the_ownership_marker() {
    ...
    assert!(
        formula
            .starts_with("# Generated by shipshape; do not edit by hand (template-version: 2)\n"),
...
#[test]
#[ignore = "source-formula migration behavior was intentionally retired"]
fn homebrew_tap_write_surgically_edits_a_hand_maintained_formula() {
    ...
    // hand-managed).
    assert!(
        !written.contains("# Generated by shipshape"),
...
fn homebrew_tap_write_refuses_an_unmarked_formula_with_no_url_stanza() {
    ...
    assert!(
        matches!(&err, AdapterError::Command { stderr, .. }
            if stderr.contains("shipshape ownership marker") && stderr.contains("found 0 `url`")),
...
fn homebrew_marker_is_recognised_only_on_the_first_line() {
    ...
    // treated as shipshape-managed and full-regenerated — it takes the surgical path,
    ...
    let current = "class Hbfake < Formula\n\
         ...
         \x20 # historical note: # Generated by shipshape; do not edit by hand (template-version: 1)\n\
    ...
    assert!(
        !written.starts_with("# Generated by shipshape"),
...
fn homebrew_tap_creates_the_initial_formula_when_absent() {
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.contains("checkout -b shipshape-homebrew-hbcreate-1.2.3")),
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.contains("push --set-upstream origin shipshape-homebrew-hbcreate-1.2.3")),
    ...
    assert!(formula.contains("bin.install \"hbcreate\""), "{formula}");
    assert!(formula.contains("sha256 \""), "{formula}");
...
#[test]
fn homebrew_prebuilt_formula_renders_each_supported_platform_without_rust() {
    let sha = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
    let assets = [
        HomebrewAsset {
            triple: "aarch64-apple-darwin".into(),
        ...
        HomebrewAsset {
            ...
            url: "https://example/aarch64-unknown-linux-musl.tar.xz".into(),
            sha256: sha.into(),
        },
        HomebrewAsset {
    ...
    assert!(
        !formula.contains("cargo") && !formula.contains("depends_on \"rust\""),
        "{formula}"
    );
}
...
fn homebrew_render_formula_neutralizes_ruby_interpolation() {
    ...
    let formula = super::homebrew::render_formula(
        ...
        &[HomebrewAsset {
            triple: "aarch64-apple-darwin".into(),
            url: "https://github.com/o/r/releases/download/v1.0.0/tool-aarch64-apple-darwin.tar.xz"
                .into(),
            sha256: "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef".into(),
        }],
        Some(malicious),
