...
fn complete_formula(version: &str, marker: bool) -> String {
...
impl FakeCmd {
    ...
    fn homebrew_observation(&self, program: &str, args: &[&str]) -> Option<CommandOutput> {
        ...
        .then(|| CommandOutput {
            status: Some(0),
            stdout: complete_formula(&self.publish_version, true),
            stderr: String::new(),
        })
    }
    ...
    fn run(&self, program: &str, args: &[&str], cwd: &Path) -> io::Result<CommandOutput> {
        ...
        if program == "gh" && args.starts_with(&["release", "view"]) && args.contains(&"--json") {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: format!(
                    r#"{{"tagName":"{}","isDraft":false,"assets":[{{"name":"dist-manifest.json"}},{{"name":"tool-aarch64-apple-darwin.tar.xz"}},{{"name":"tool-aarch64-unknown-linux-musl.tar.xz"}},{{"name":"tool-x86_64-unknown-linux-musl.tar.xz"}},{{"name":"shipshape-aarch64-apple-darwin.tar.xz"}},{{"name":"shipshape-aarch64-unknown-linux-musl.tar.xz"}},{{"name":"shipshape-x86_64-unknown-linux-musl.tar.xz"}}]}}"#,
                    args[2]
                ),
                stderr: String::new(),
            });
        ...
        if program == "gh" && args.starts_with(&["release", "download"]) {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: format!(
                    r#"{{"announcement_tag":"{}","releases":[{{"app_name":"tool","app_version":"{}","artifacts":["tool-aarch64-apple-darwin.tar.xz","tool-aarch64-unknown-linux-musl.tar.xz","tool-x86_64-unknown-linux-musl.tar.xz"]}},{{"app_name":"shipshape","app_version":"{}","artifacts":["shipshape-aarch64-apple-darwin.tar.xz","shipshape-aarch64-unknown-linux-musl.tar.xz","shipshape-x86_64-unknown-linux-musl.tar.xz"]}}]}}"#,
...
impl RegistryQuery for FakeRegistry {
    fn http_get(&self, _url: &str) -> io::Result<(u16, Vec<u8>)> {
        ...
        Ok((200, complete_formula(&version, true).into_bytes()))
    }
...
impl RegistryQuery for UnmarkedFormulaRegistry {
    fn http_get(&self, _url: &str) -> io::Result<(u16, Vec<u8>)> {
        let intel = if self.intel {
            format!(
                "    if Hardware::CPU.intel?\n      url \"https://example/tool-x86_64-apple-darwin.tar.xz\"\n      sha256 \"{CANNED_SHA256}\"\n    end\n"
            )
        } else {
            String::new()
        };
...
impl CommandRunner for WorkspaceCmd {
    fn run(&self, program: &str, args: &[&str], _cwd: &Path) -> io::Result<CommandOutput> {
        ...
        if program == "gh" && args.starts_with(&["release", "view"]) && args.contains(&"--json") {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: r#"{"tagName":"v1.2.3","isDraft":false,"assets":[{"name":"dist-manifest.json"},{"name":"shipshape-aarch64-apple-darwin.tar.xz"},{"name":"shipshape-aarch64-unknown-linux-musl.tar.xz"},{"name":"shipshape-x86_64-unknown-linux-musl.tar.xz"}]}"#.into(),
                stderr: String::new(),
            });
        }
        if program == "gh" && args.starts_with(&["release", "download"]) {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: r#"{"announcement_tag":"v1.2.3","releases":[{"app_name":"shipshape","app_version":"1.2.3","artifacts":["shipshape-aarch64-apple-darwin.tar.xz","shipshape-aarch64-unknown-linux-musl.tar.xz","shipshape-x86_64-unknown-linux-musl.tar.xz"]}]}"#.into(),
                stderr: String::new(),
            });
        }
                ...
                .is_some_and(|url| url.contains("raw.githubusercontent.com"))
        {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: complete_formula("1.2.3", true),
                stderr: String::new(),
            });
        }
...
impl RegistryQuery for WorldRegistry {
    fn http_get(&self, _url: &str) -> io::Result<(u16, Vec<u8>)> {
        Ok((200, complete_formula("1.2.3", true).into_bytes()))
    }
...
fn engine_tap_artifacts() -> ReleaseArtifacts {
    ReleaseArtifacts {
        ...
        .map(|triple| HomebrewAsset {
            triple: triple.into(),
            url: format!("https://example/tool-{triple}.tar.xz"),
...
fn ci_delegated_homebrew_skips_dist_write_but_verifies_unmarked_formula() {
    ...
    let idgen = FakeIdGen("RUN01".into());
    let cmd = FakeCmd::new();
    let registry = UnmarkedFormulaRegistry { intel: false };
    let tagger = FakeTagger::new();
    let root = PathBuf::from("/repo");
    let ctx = EffectCtx {
        runner: &cmd,
...
#[test]
fn ci_delegated_homebrew_rejects_an_extra_intel_stanza_but_preserves_history() {
    let cmd = FakeCmd::new();
    let clock = FakeClock(Cell::new(1000));
    let registry = UnmarkedFormulaRegistry { intel: true };
    let root = PathBuf::from("/repo");
    let ctx = EffectCtx {
        runner: &cmd,
        clock: &clock,
    ...
    assert_eq!(
        ...
        "a current three-platform plan cannot silently claim Intel"
    );

    plan.homebrew_platforms
        .insert(1, "x86_64-apple-darwin".into());
    assert_eq!(
        verify_delegated_homebrew(&ctx, &plan, "tool"),
        VerifyOutcome::Matches,
        "a historical four-platform plan is verified against its sealed promise"
    );
}
