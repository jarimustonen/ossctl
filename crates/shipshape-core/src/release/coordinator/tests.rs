...
struct FakeCmd {
    ...
    transient_fail_contains: Option<String>,
    transient_failures_remaining: Cell<u32>,
...
impl FakeCmd {
    fn new() -> Self {
        Self {
            ...
            fail_contains: None,
            transient_fail_contains: None,
            transient_failures_remaining: Cell::new(0),
            origin: None,
    ...
    fn failing_once_on(substr: &str) -> Self {
        Self::failing_n_times_on(substr, 1)
    }
    fn failing_n_times_on(substr: &str, attempts: u32) -> Self {
        Self {
            transient_fail_contains: Some(substr.to_string()),
            transient_failures_remaining: Cell::new(attempts),
            ..Self::new()
        }
    }
    ...
    fn transient_failure(&self, line: &str) -> Option<CommandOutput> {
        let remaining = self.transient_failures_remaining.get();
        (remaining > 0
            && self
                .transient_fail_contains
                .as_ref()
                .is_some_and(|needle| line.contains(needle)))
        .then(|| {
            self.transient_failures_remaining.set(remaining - 1);
            CommandOutput {
                status: Some(1),
                stdout: String::new(),
    ...
    fn clone_tap_checkout(&self, program: &str, args: &[&str]) -> Option<CommandOutput> {
        if program != "gh" || !args.starts_with(&["repo", "clone"]) {
            return None;
        }
        let workdir = Path::new(args.get(3)?);
        std::fs::create_dir_all(workdir.join("Formula")).ok()?;
        std::fs::write(
            workdir.join("Formula/tool.rb"),
            format!(
                "# Generated by shipshape; do not edit by hand (template-version: 2)\n\
                 class Tool < Formula\n  version \"{}\"\n  url \"https://example/old.tar.xz\"\n  sha256 \"{CANNED_SHA256}\"\nend\n",
    ...
    fn run(&self, program: &str, args: &[&str], cwd: &Path) -> io::Result<CommandOutput> {
        ...
        if program == "gh" && args.starts_with(&["release", "view"]) && args.contains(&"--json") {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: format!(
                    r#"{{"tagName":"{}","isDraft":false,"assets":[{{"name":"dist-manifest.json"}},{{"name":"tool-aarch64-apple-darwin.tar.xz"}},{{"name":"tool-x86_64-apple-darwin.tar.xz"}},{{"name":"tool-aarch64-unknown-linux-musl.tar.xz"}},{{"name":"tool-x86_64-unknown-linux-musl.tar.xz"}},{{"name":"shipshape-aarch64-apple-darwin.tar.xz"}},{{"name":"shipshape-x86_64-apple-darwin.tar.xz"}},{{"name":"shipshape-aarch64-unknown-linux-musl.tar.xz"}},{{"name":"shipshape-x86_64-unknown-linux-musl.tar.xz"}}]}}"#,
                    args[2]
                ),
                stderr: String::new(),
            });
        ...
        if program == "gh" && args.starts_with(&["release", "download"]) {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: format!(
                    r#"{{"announcement_tag":"{}","releases":[{{"app_name":"tool","app_version":"{}","artifacts":["tool-aarch64-apple-darwin.tar.xz","tool-x86_64-apple-darwin.tar.xz","tool-aarch64-unknown-linux-musl.tar.xz","tool-x86_64-unknown-linux-musl.tar.xz"]}},{{"app_name":"shipshape","app_version":"{}","artifacts":["shipshape-aarch64-apple-darwin.tar.xz","shipshape-x86_64-apple-darwin.tar.xz","shipshape-aarch64-unknown-linux-musl.tar.xz","shipshape-x86_64-unknown-linux-musl.tar.xz"]}}]}}"#,
        ...
        if program == "shasum" || program == "sha256sum" {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: format!("{CANNED_SHA256}  /tmp/shipshape-src-tarball.tar.gz"),
                stderr: String::new(),
            });
        }
...
/// A cargo-dist-owned formula has no shipshape marker, but is otherwise complete.
...
fn homebrew_fetches_only_servable_platforms_from_the_full_distribution_set() {
    // This is shipshape's own distribution shape: cargo-dist also publishes Windows
...
/// `git`-less command runner over a two-crate workspace (`shipshape-core` ← `shipshape`)
...
struct WorkspaceCmd {
    ...
    /// Exact-match (not substring) so failing `… -p shipshape` cannot also catch
    /// `… -p shipshape-core`. A failed publish does **not** mark its crate published.
...
impl CommandRunner for WorkspaceCmd {
    fn run(&self, program: &str, args: &[&str], _cwd: &Path) -> io::Result<CommandOutput> {
        ...
        if program == "gh" && args.starts_with(&["release", "view"]) && args.contains(&"--json") {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: r#"{"tagName":"v1.2.3","isDraft":false,"assets":[{"name":"dist-manifest.json"},{"name":"shipshape-aarch64-apple-darwin.tar.xz"}]}"#.into(),
                stderr: String::new(),
            });
        }
        if program == "gh" && args.starts_with(&["release", "download"]) {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: r#"{"announcement_tag":"v1.2.3","releases":[{"app_name":"shipshape","app_version":"1.2.3","artifacts":["shipshape-aarch64-apple-darwin.tar.xz"]}]}"#.into(),
                stderr: String::new(),
            });
        }
        ...
        if program == "git" && args == ["remote", "get-url", "origin"] {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: "git@github.com:jarimustonen/shipshape.git".to_string(),
                stderr: String::new(),
            });
        }
        if program == "curl"
                ...
                .is_some_and(|url| url.contains("raw.githubusercontent.com"))
        {
            return Ok(CommandOutput {
                status: Some(0),
                stdout: "# Generated by shipshape; do not edit by hand (template-version: 2)\nclass Ossctl < Formula\n  version \"1.2.3\"\n  if OS.mac? && Hardware::CPU.arm?\n    url \"https://example/shipshape-aarch64-apple-darwin.tar.xz\"\n    sha256 \"deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef\"\n  end\nend\n".into(),
                stderr: String::new(),
            });
        }
        ...
        // and byte-compares. Fake the clone by checking out an OLDER *shipshape-marked*
        // formula (shipshape's own tap formula is shipshape-generated) so the rendered one
        ...
        if program == "gh" && args.first() == Some(&"repo") && args.get(1) == Some(&"clone") {
            if let Some(workdir) = args.get(3) {
                ...
                std::fs::write(
                    dir.join("shipshape.rb"),
                    "# Generated by shipshape; do not edit by hand (template-version: 1)\n\
                     class Ossctl < Formula\n  # old\nend\n",
                )
                .unwrap();
            }
        ...
        if program == "cargo" && args.contains(&"metadata") {
            // Two publishable members: `shipshape` depends on `shipshape-core`.
            return Ok(CommandOutput {
                status: Some(0),
                stdout: r#"{"packages":[
                    {"name":"shipshape-core","version":"1.2.3","id":"shipshape-core 1.2.3","dependencies":[],"publish":null},
                    {"name":"shipshape","version":"1.2.3","id":"shipshape 1.2.3","dependencies":[{"name":"shipshape-core"}],"publish":null}
                ],"workspace_members":["shipshape-core 1.2.3","shipshape 1.2.3"]}"#.to_string(),
                stderr: String::new(),
            });
        }
...
fn two_crates_io_targets_in_one_ecosystem_cut_in_dependency_order() {
    ...
    // crates.io targets in the `rust` ecosystem (`shipshape-core`, then `shipshape`) is
    ...
    let plan = ReleasePlan {
        ...
        head_sha: "deadbeef".into(),
        version: "1.2.3".into(),
        targets: vec![crates_target("shipshape-core"), crates_target("shipshape")],
        phases: PlanPhase::SEQUENCE.to_vec(),
        bump: None,
        homebrew_tap: None,
        license: None,
    ...
    assert_eq!(ids, vec!["rust:shipshape-core", "rust:shipshape"]);
    ...
    assert!(state.published.contains_key("rust:shipshape-core"));
    assert!(state.published.contains_key("rust:shipshape"));
    assert_eq!(state.published.len(), 2);

    // Dependency order at the process level: `shipshape-core` publishes before
    // `shipshape`, and exactly once (ADR-0004: each target publishes only its own
    // crate — `shipshape`'s target waits for `shipshape-core` to index, never re-publishes
    // it).
    let calls = cmd.calls();
    let core_pub = calls
        .iter()
        .position(|c| c == "cargo publish --registry crates-io -p shipshape-core")
        .expect("shipshape-core was published");
    let cli_pub = calls
        .iter()
        .position(|c| c == "cargo publish --registry crates-io -p shipshape")
        .expect("shipshape was published");
    assert!(
        core_pub < cli_pub,
    ...
    assert_eq!(
        calls
            .iter()
            .filter(|c| *c == "cargo publish --registry crates-io -p shipshape-core")
            .count(),
        1,
...
fn two_targets_do_not_double_publish_a_shared_dependency_under_index_lag() {
    ...
    // COORDINATOR LEVEL: two crates.io targets `shipshape-core` then `shipshape` cut in
    // dependency order, where `shipshape-core` stays index-lagged for several polls
    // after it publishes. The old model re-ran `cargo publish -p shipshape-core` from
    // `shipshape`'s closure during that lag (a partial-publish trap); ADR-0004's
    ...
    // dependent's target ran — never a second `cargo publish -p shipshape-core`.
    assert_eq!(
        calls
            .iter()
            .filter(|c| *c == "cargo publish --registry crates-io -p shipshape-core")
    ...
    assert_eq!(
        calls
            .iter()
            .filter(|c| *c == "cargo publish --registry crates-io -p shipshape")
            .count(),
        1
    );
...
fn dependent_packaging_interleaves_into_publish_not_build_all() {
    ...
    // targets: the leaf `shipshape-core`, then the dependent `shipshape` (which pins
    // `shipshape-core = "=1.2.3"`). The dependent CANNOT be `cargo package`d in build-all
    ...
    // in the dep-ordered publish phase, which runs after `shipshape-core` is published
    ...
    assert!(
        !calls
            .iter()
            .any(|c| c == "cargo package --registry crates-io -p shipshape --no-verify"),
    ...
    assert!(
        calls
            .iter()
            .any(|c| c == "cargo package --registry crates-io -p shipshape-core --no-verify"),
        "the leaf was not packaged in build-all: {calls:?}"
    );
    // The interleave order: `shipshape-core` publishes, then `shipshape` publishes (its
...
fn resume_after_core_publish_completes_the_dependent_without_republishing_core() {
    ...
    // `shipshape-core` but BEFORE publishing the dependent `shipshape` must resume —
    // skipping the already-landed, irreversible `shipshape-core` and completing `shipshape`.
    // The journal (per-target `published` set) and the crates world are shared across
    // the two runs; the second run must never re-run `cargo publish -p shipshape-core`.
    let world = CratesWorld::default();
    let store = FakeStore::default();
    let clock = FakeClock(Cell::new(1000));
    let idgen = FakeIdGen("RUN01".into());
    ...
    // First run: the dependent's publish dies. `shipshape-core` lands (published +
    // journalled); the publish phase fails before `shipshape`.
    let reg = WorldRegistry {
        world: world.clone(),
    };
    let failing = WorkspaceCmd::failing_line(
        world.clone(),
        "cargo publish --registry crates-io -p shipshape",
    );
    ...
    assert!(
        journal
            .state()
            .published
            .contains_key("rust:shipshape-core"),
        "the dependency did not land before the interruption"
    );
    assert!(
        !journal.state().published.contains_key("rust:shipshape"),
        "the dependent must not be recorded as published after a failed publish"
    );
    assert!(world.published.borrow().contains("shipshape-core"));
    assert!(!world.published.borrow().contains("shipshape"));

    // Second run: a healthy runner over the SAME journal + crates world. Resume must
    // skip `shipshape-core` (already published) and land `shipshape`.
    ...
    assert_eq!(journal.state().status, RunStatus::Completed);
    assert!(journal.state().published.contains_key("rust:shipshape"));
    ...
    // `cargo publish -p shipshape-core` would hard-fail on crates.io and is irreversible).
    assert!(
        !resume_calls
            .iter()
            .any(|c| c == "cargo publish --registry crates-io -p shipshape-core"),
        "resume re-published the already-landed dependency: {resume_calls:?}"
    );
    // The dependent's publish completes on resume — now that `shipshape-core` is on the
    // index, its deferred packaging succeeds inside `cargo publish`.
    assert!(
        resume_calls
            .iter()
            .any(|c| c == "cargo publish --registry crates-io -p shipshape"),
...
fn engine_tap_plan() -> ReleasePlan {
    let mut plan = homebrew_core_plan();
    plan.targets[0].adapter = Adapter::HomebrewTap;
    plan.homebrew_tap = Some("owner/homebrew-tool".into());
    plan
}

fn engine_tap_artifacts() -> ReleaseArtifacts {
    ReleaseArtifacts {
        source_tarball: Some(SourceTarball {
            url: "https://example/source.tar.gz".into(),
            sha256: Some(CANNED_SHA256.into()),
        }),
        repo_slug: Some("owner/tool".into()),
        homebrew: Some(HomebrewFormula {
            tap: Some("owner/homebrew-tool".into()),
            version: "1.2.3".into(),
            platforms: vec!["aarch64-apple-darwin".into()],
            ..HomebrewFormula::default()
        }),
        homebrew_assets: vec![HomebrewAsset {
            triple: "aarch64-apple-darwin".into(),
            url: "https://example/tool-aarch64-apple-darwin.tar.xz".into(),
            sha256: CANNED_SHA256.into(),
        }],
...
#[test]
fn dist_publish_retries_a_transient_clone_failure() {
    let cmd = FakeCmd::failing_once_on("gh repo clone");
    let clock = FakeClock(Cell::new(1000));
    let registry = FakeRegistry::empty();
    let artifacts = engine_tap_artifacts();
    let ctx = EffectCtx {
        runner: &cmd,
        clock: &clock,
        registry: &registry,
    ...
    let targets = resolve_target_plans(&engine_tap_plan()).unwrap();

    publish_dist_with_retry(&ctx, &targets[0]).expect("the retry should recover the clone 503");

    assert_eq!(
        cmd.calls()
            .iter()
            .filter(|call| call.starts_with("gh repo clone"))
            .count(),
        2,
        "the transient clone failure must be retried exactly once before success"
    );
    assert_eq!(clock.0.get(), 1003, "one 2s sleep plus receipt timestamp");
}

#[test]
fn dist_publish_stops_after_the_bounded_clone_retries() {
    let cmd = FakeCmd::failing_n_times_on("gh repo clone", DIST_PUBLISH_ATTEMPTS);
    let clock = FakeClock(Cell::new(1000));
    let registry = FakeRegistry::empty();
    let artifacts = engine_tap_artifacts();
    let ctx = EffectCtx {
    ...
    publish_dist_with_retry(&ctx, &targets[0]).expect_err("three clone 503s exhaust retry");
    ...
    assert_eq!(
        cmd.calls()
            ...
            .filter(|call| call.starts_with("gh repo clone"))
            .count(),
        DIST_PUBLISH_ATTEMPTS as usize
    );
    assert_eq!(clock.0.get(), 1006, "backoff must be exactly 2s then 4s");
}
...
fn dist_failure_after_tag_still_runs_verify_and_reports_observations() {
    ...
    assert_eq!(
        ...
        "a failed dist barrier must never become Completed through observation"
    );
    assert_eq!(
        cmd.calls()
            .iter()
            ...
            .count(),
        1,
        "a mutating command with ambiguous disposition must not be blindly retried"
    );
    ...
    assert!(
        error.to_string().contains("post-failure verify ran")
            ...
            && error.to_string().contains("verify-phase failed"),
...
#[test]
fn shipshape_like_contract_cuts_end_to_end_across_target_classes() {
    // The `release-engine-cut-cargo-dist-flow` acceptance: shipshape's own contract —
    // two crates.io targets (shipshape-core → shipshape), a gh-releases cargo-dist target
    ...
    let shipshape_target = |registry, adapter| PlanTarget {
        ecosystem: Ecosystem::Rust,
    ...
    let plan = ReleasePlan {
        plan_id: "plan-shipshape".into(),
        contract_schema_version: 1,
        head_sha: "deadbeef".into(),
        version: "1.2.3".into(),
        targets: vec![
            crates_target("shipshape-core"),
            crates_target("shipshape"),
            shipshape_target(Registry::GhReleases, Adapter::CargoDist),
            shipshape_target(Registry::Homebrew, Adapter::HomebrewTap),
        ],
        phases: PlanPhase::SEQUENCE.to_vec(),
        bump: None,
        homebrew_tap: Some("jarimustonen/homebrew-shipshape".into()),
        license: Some("MIT".into()),
        description: Some("Test release tool".into()),
        homebrew_platforms: vec!["aarch64-apple-darwin".into()],
    };
    let ids = crate::release::journal_target_ids(&plan.targets);
    // ids: [shipshape-core@crates.io, shipshape@crates.io, shipshape@gh-releases, shipshape@homebrew]
    let mut journal = Journal::create(
        &store,
        &clock,
        &idgen,
    ...
    assert!(
        calls
            .iter()
            .any(|c| c.starts_with("gh repo clone jarimustonen/homebrew-shipshape ")),
    ...
    let hb_prefix = "shipshape-homebrew-shipshape-1.2.3-";
    ...
    let formula = std::fs::read_to_string(hb_dir.join("Formula/shipshape.rb"))
    ...
    assert!(
        formula.contains(
            "url \"https://github.com/jarimustonen/shipshape/releases/download/v1.2.3/shipshape-aarch64-apple-darwin.tar.xz\""
        ),
...
fn seed_workspace() -> TempDir {
    let dir = tempfile::Builder::new()
        .prefix("shipshape-coord-bump-seed-")
        .tempdir()
        .unwrap();
    let root = dir.path();
