...
- **`taskfleet`** — `taskfleet-core` (lib) + `taskfleet-cli` (bin); **bundles an entire `/worktree-*` skill family** (18 skills) under `crates/taskfleet-cli/skills/<name>/SKILL.template.md`, version-pinned, installed via `taskfleet skill install`; and an **event-sourced run journal** in `taskfleet-core` (`manifest.json` + append-only events + an `applied_seq` watermark + an idempotent reducer, with append-then-apply atomicity).
...
- **`ossctl-core` carries many concerns.** Accepted at founding (matches taskfleet-core) and bounded by strict internal module boundaries + injected ports; the domain-crate split remains a cheap later move if it hurts (a documented, non-breaking promotion).
