...
- When the plan carries **≥1 CI-delegated target** (the same `is_ci_delegated()` classification `release-engine-cut-cargo-dist-flow` introduced, e.g. `cargo-dist`), the coordinator **creates and pushes the tag but does NOT create the GitHub Release** — the pushed tag is what triggers CI, and CI owns Release creation + the cross-platform binary upload. The delegation is journalled as `github_release_delegated` (in place of `github_release_created`), so resume/verify treat the missing engine-created Release as intentional, never a step to re-attempt.
- With **no** CI-delegated target, the coordinator still creates the Release itself — unchanged ADR-0002 behavior.
