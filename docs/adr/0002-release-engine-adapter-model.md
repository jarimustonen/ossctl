...
- The **cargo adapter** classifies each target by whether it has publishable intra-workspace dependencies (read-only `cargo metadata`). A **leaf** (no such deps) is packaged in build-all exactly as before (`cargo check` then `cargo package --no-verify`, producing the `.crate`). A **`=`-pinned dependent** runs only the **index-independent `cargo check`** in dry-run/build (the sibling resolves via its on-disk `path`, never the index), and its **packaging is deferred to `cargo publish`** in publish-all — which packages **and** publishes as one unit, *after* the dependency is published and index-visible.
...
- The **pre-publish compile safety net is still a global build-all barrier.** Every target's `cargo check` runs before **any** irreversible publish, so a genuine compile error (type/trait/API mismatch, missing item) fails the cut before anything lands — the partial-publish trap ADR-0004 exists to prevent. Only the dependent's *packaging* (which cargo fuses with publishing anyway) moves into the publish phase.
...
**Residual, unavoidable risk (documented).** A `=`-pinned dependent's *manifest/packaging* validity (bad `license`, an excluded required file) can only be checked by `cargo package`/`cargo publish`, which cannot run until the dependency is on the index — so such an error surfaces at the dependent's `cargo publish`, **after** its dependency has published (a torn release the engine journals but cannot roll back). This is inherent to cargo's model — the manual fallback recipe carries the identical risk — and it is bounded by the index-independent `cargo check`, which still catches every *compile* error up front. A plan-time coverage/packaging preflight that narrows this further is a follow-up, not owned here.
