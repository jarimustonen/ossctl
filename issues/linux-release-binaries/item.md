...
- taskfleet v0.1.0 → `taskfleet-{x86_64,aarch64}-unknown-linux-gnu.tar.xz` + installer

## Impact

maintainer's homebase provisions issuectl/ossctl/taskfleet on every machine via
...
taskfleet hooks fall back to the cargo-dist release installer, but the
...
issuectl/taskfleet already do (same cargo-dist setup). Also worth adding
