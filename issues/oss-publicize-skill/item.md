...
# Collect de-stealth experience toward an /oss-publicize skill
...
attract external users. The session used the existing `/oss-*` members plus a set
of manual steps no member owns. This issue is the **collection point** for that
...
1. **README rewrite for an external audience** (`/oss-readme`, but as a full
...
4. **GitHub repo metadata applied, not just suggested** — description and topics
   were both empty; `/oss-readme` only prints `gh repo edit` guidance. On the
   maintainer's go the commands were executed.
5. **`/oss-contributing`** — worked as designed, with two owner corrections:
...
6. **`/oss-security-policy`** — threat gate fired (subprocess probes, untrusted
...
## Gaps in the current /oss-* family this exposed

- No member **applies** repo metadata (description/topics/social preview) —
  `/oss-readme` stops at printed guidance. Fine as a default, but the publicize
  pass wants an "apply on explicit go" step.
...
- No member does the **claims-vs-binary audit** of an existing README (step 2).
  `/oss-readme` trusts contract + facts; the drift that matters was between
...
- **A (lean): a new thin member, working name `/oss-publicize`** — an
...
- **B: a mode of `/oss-release`** — the family already has one orchestrator;
...
- **C: no skill, just a checklist doc** — cheapest, but the pass has real
  ...
  automation in `ossctl` (e.g. an `ossctl audit` dimension: "readme claims match
  distribution surface", "PVR enabled when SECURITY.md references it").
